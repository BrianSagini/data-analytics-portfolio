...
- **Real `.pbip` project files now exist for all 4 reports** under `powerbi/0N_*/*.pbip`,
  generated by `scripts/gen_pbip.py` — **but none has ever been opened in Power BI Desktop, so
  none has been validated.** Earlier in this project, opening Power BI Desktop to validate a file
  via screenshot was attempted once and immediately stopped: this machine is a live, actively-used
  desktop, not an isolated environment, and the resulting screenshot captured unrelated personal
  content (browser tabs, job-search activity) that had nothing to do with Power BI. That
  screenshot was deleted immediately and never left the local filesystem (confirmed never staged
  in git) — see `docs/final_release_audit_report.md`'s Power BI section for the full account. The
  project owner explicitly asked for real files to be hand-authored anyway, accepting that they
  cannot be opened/validated from this session — so what exists now is exactly that: a genuine,
  best-effort, unvalidated artifact, not a claim that it works.
  - **What's real and complete in each `.pbip`**: the full semantic model — every table matching
    its source view's actual columns (verified against live `information_schema`, not guessed),
    correct data types, every DAX measure from this guide, and the relationships between tables.
    The underlying M query in every table's partition
    (`PostgreSQL.Database("localhost:5433", "analytics")`) is real, stable, documented Power Query
    syntax.
  - **What's a placeholder**: each report's 4 pages exist (correctly named, matching this guide)
    but contain **no visuals** — hand-authoring the visual-JSON layer blind was judged the single
    highest-risk, least-verifiable part of the format (unlike the table/measure TMDL, which is
    straightforward enough to review by inspection), so it was deliberately left for a person to
    populate interactively — fast, since the entire data model and every measure already exists
    and is correct; it's drag-and-drop from here, not re-deriving anything.
  - **Whether it will actually open in Power BI Desktop without error is genuinely unknown** —
    every JSON file was validated to at least parse as syntactically correct JSON
    (`scripts/gen_pbip.py`'s output checked with `json.load`), but that doesn't confirm Power BI's
    own schema requirements are satisfied, and the TMDL files have no equivalent local validator
    at all. The first real test is a person opening `powerbi/0N_*/<Name>.pbip` in Desktop.
...
| Project | File | Semantic model | Report pages |
|---|---|---|---|
| 1. Climate Risk | `powerbi/01_climate_risk_business_impact/ClimateRisk.pbip` | 3 tables, 8 measures, 2 relationships — all real | 4 pages present, empty |
| 2. Dark Store | `powerbi/02_dark_store_intelligence/DarkStoreIntelligence.pbip` | 4 tables (incl. a real `Stores` dimension from `dark_store.stores`), 8 measures, 3 relationships | 4 pages present, empty |
| 3. Hiring Bias | `powerbi/03_ai_hiring_bias_detector/HiringBiasDetector.pbip` | 3 tables, 11 measures, 0 relationships (by design — see report 3's section) | 4 pages present, empty |
| 4. Fraud Pattern | `powerbi/04_fraud_pattern_evolution/FraudPatternEvolution.pbip` | 3 tables, 9 measures, 0 relationships (by design) | 4 pages present, empty |

To use one: double-click the `.pbip` file (or open it from Power BI Desktop's File menu). **Get
Data, table import, relationships, and every measure below are already built into the file** —
what's left is purely visual layout, using the tables/measures/page names already there. If the
file fails to open at all, that means something in the hand-authored TMDL/PBIR is wrong; the
per-report sections below still serve as a complete from-scratch guide in that case (Get Data →
PostgreSQL → the same tables/measures/pages, built fresh in the UI).
